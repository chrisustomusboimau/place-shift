...
def create_staff(data: StaffCreate, session: Session = Depends(get_session), _=Depends(require_admin)):
    ...
    if data.username:
        existing_user = session.exec(select(Staff).where(Staff.username == data.username.strip())).first()
        if existing_user:
            raise HTTPException(
    ...
    # FIX: Memasukkan objek data murni langsung ke model Staff
    s = Staff(**staff_dict)
    session.add(s)
    session.commit()
    session.refresh(s)
...
def update_staff(
    ...
    # FIX: Langsung perbarui properti profil tanpa saringan filter cache memory
